...
Run:
  ...
  python api.py          (in a separate terminal first)
  python test_e2e.py
...
def test_mongo_record_count():
    client = pymongo.MongoClient(MONGO_URI)
    count  = client["smart_home"]["appliance_data"].count_documents({})
...
def test_mongo_devices_present():
    client  = pymongo.MongoClient(MONGO_URI)
    devices = client["smart_home"]["appliance_data"].distinct("device_name")
...
def test_mongo_schema_fields():
    client = pymongo.MongoClient(MONGO_URI)
    sample = client["smart_home"]["appliance_data"].find_one({}, {"_id": 0})
...
def test_mongo_no_null_power():
    client = pymongo.MongoClient(MONGO_URI)
    nulls  = client["smart_home"]["appliance_data"].count_documents(
...
def test_mongo_alerts_written():
    client = pymongo.MongoClient(MONGO_URI)
    count  = client["smart_home"]["anomaly_alerts"].count_documents({})
...
def test_mongo_alert_schema():
    client = pymongo.MongoClient(MONGO_URI)
    alert  = client["smart_home"]["anomaly_alerts"].find_one({}, {"_id": 0})
...
def test_mongo_checkpoint_written():
    client = pymongo.MongoClient(MONGO_URI)
    chk    = client["smart_home"]["infer_checkpoint"].find_one()
...
def test_feature_no_nan():
    client  = pymongo.MongoClient(MONGO_URI)
    records = list(client["smart_home"]["appliance_data"].find({}, {"_id": 0}).limit(500))
...
def test_model_contamination_rate():
    client  = pymongo.MongoClient(MONGO_URI)
    records = list(client["smart_home"]["appliance_data"].find(
...
def test_ui_energy_trend_daily():
    ...
    for ds in data["chart_data"]["datasets"]:
        assert len(ds["data"]) == n, f"Dataset length mismatch in daily trend"
...
def test_cross_total_energy_matches():
    client = pymongo.MongoClient(MONGO_URI)
    mongo_total = next(client["smart_home"]["appliance_data"].aggregate([
...
def test_cross_alert_count_matches():
    client      = pymongo.MongoClient(MONGO_URI)
    mongo_count = client["smart_home"]["anomaly_alerts"].count_documents({})
...
def test_cross_device_list_matches():
    client         = pymongo.MongoClient(MONGO_URI)
    mongo_devices  = sorted(client["smart_home"]["appliance_data"].distinct("device_name"))
