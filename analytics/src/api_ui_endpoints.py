    ...
    recent_alerts = [
        {
            ...
            "label":          a.get("anomaly_type") or a.get("ground_truth_anomaly") or "unusual_pattern",
        }
        for a in recent_raw
    ]
...
def ui_anomaly_feed():
    ...
    if sev_filter:
        if sev_filter not in SEVERITY_THRESHOLDS:
            return err("severity must be one of: critical, high, medium, low")
        ranges = {
    ...
    for a in raw:
        ...
        gt  = a.get("anomaly_type") or a.get("ground_truth_anomaly") or "unusual_pattern"
