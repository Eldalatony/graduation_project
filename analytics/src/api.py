...
api.py
------
Core analytics REST API for the Smart Home Energy Monitoring System.

Severity levels on every anomaly response, anomaly-rate %, daily pagination,
input validation, structured request logging, and consistent rounding/shape.
The UI-shaped (/api/ui/*) and HE (/api/he/*) endpoints live in their own
blueprints (api_ui_endpoints.py, he_layer.py); app.py is the service entry point.
...
Endpoints:
  ...
  GET /api/analytics/bill-estimate             ?days  ?method=flat|seasonal
  GET /api/analytics/:applianceId/forecast     ?hours
"""
...
from shared import (
    db, data_col, alerts_col,
    SEVERITY_THRESHOLDS, score_to_severity,
)
from model_manager import all_device_status
from forecaster import forecast_device
...
try:
    ...
    log.info("HE blueprint registered")
...
def _load_device_df(device_name: str):
    """Pull one device's full history as a sorted DataFrame for forecasting."""
...
def devices_status():
    ...
    learning = [s for s in statuses if s["status"] != "ready"]
    return ok({
...
def bill_estimate():
    ...
    method = (request.args.get("method") or "flat").lower()
    if method not in ("flat", "seasonal"):
        return err("'method' must be 'flat' or 'seasonal'")
    if method == "seasonal":
        return _bill_estimate_seasonal(projection_days)
    ...
    per_device = []
    for d in device_totals:
        days_of_data = max(len(d["days"]), 1)
        daily_cost   = d["total_cost_EGP"] / days_of_data
        daily_energy = d["total_energy"]   / days_of_data
        per_device.append({
            "device_name":          d["_id"],
...
def _bill_estimate_seasonal(projection_days: int):
    ...
    instead of multiplying a single flat daily average. Captures daily/weekly
    peaks and weekends, which the flat method averages away."""
    horizon_hours = projection_days * 24
    devices = data_col.distinct("device_name")
    if not devices:
        return err("Not enough data to estimate bill")

    per_device, total_cost, total_kwh = [], 0.0, 0.0
    for device in devices:
        device_df = _load_device_df(device)
        if device_df is None:
            continue
        fc = forecast_device(device, device_df, horizon_hours)
        total_cost += fc["total_predicted_cost_EGP"]
        total_kwh  += fc["total_predicted_kWh"]
        per_device.append({
            "device_name":          device,
            "method":               fc["method"],
            "projected_cost_EGP":   fc["total_predicted_cost_EGP"],
            "projected_energy_kWh": fc["total_predicted_kWh"],
            "avg_daily_cost_EGP":   round(fc["total_predicted_cost_EGP"] / projection_days, 2),
            "avg_daily_energy_kWh": round(fc["total_predicted_kWh"]  / projection_days, 4),
        })

    per_device.sort(key=lambda d: d["projected_cost_EGP"], reverse=True)
    grand = total_cost or 1
    for d in per_device:
        d["share_of_total_pct"] = round(d["projected_cost_EGP"] / grand * 100, 1)

    return ok({
        "method":                   "seasonal",
        "projection_days":          projection_days,
        "projected_total_cost_EGP": round(total_cost, 2),
        "projected_energy_kWh":     round(total_kwh, 4),
        "avg_daily_cost_EGP":       round(total_cost / projection_days, 2),
        "avg_daily_energy_kWh":     round(total_kwh  / projection_days, 4),
        "per_device":               per_device,
    })
...
if __name__ == "__main__":
    ...
    print("  GET /api/analytics/bill-estimate     ?days ?method=flat|seasonal")
    print("  GET /api/analytics/<id>/forecast     ?hours")
